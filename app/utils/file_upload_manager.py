...
from routes.api_routes.bloom_filter_routes import filter_missing_words
    ...
    try:
        ...
        try:
            ...
            for encoding in ["utf-16", "iso-8859-1", "cp1252"]:
                ...
                raise HTTPException(
                    status_code=400,
                    detail="Could not decode file. Please ensure it's a valid text file with UTF-8 encoding.",
                )

        words = text.split()
        logger.info(
            f"Successfully extracted {len(words)} words from TXT file: {file.filename}"
        )
        return words
        ...
        raise HTTPException(
            status_code=400, detail=f"Error processing TXT file: {str(e)}"
        )
...
async def extract_words_from_docx(file: UploadFile) -> list[str]:
    ...
    try:
        ...
        logger.info(
            f"Successfully extracted {len(words)} words from DOCX file: {file.filename}"
        )
        return words
        ...
        logger.error(
            f"Error reading DOCX file {file.filename}: {str(e)}", exc_info=True
        )
        raise HTTPException(
            status_code=400,
...
async def extract_words(file: UploadFile) -> list[str]:
    ...
    try:
        ...
        logger.error(
            f"Error extracting words from file {file.filename}: {str(e)}", exc_info=True
        )
...
async def filter_words_from_file(file: UploadFile) -> list[str]:
    ...
    try:
        ...
        logger.info(
            f"Filtered {len(missing_words)} missing words from {len(words)} total words"
        )
        ...
        logger.error(
            f"Error filtering words from file {file.filename}: {str(e)}", exc_info=True
        )
        raise HTTPException(status_code=500, detail=f"Error processing file: {str(e)}")
...
async def count_word_frequency(file: UploadFile) -> dict[str, int]:
    ...
    try:
        ...
        logger.info(
            f"Counted word frequency for {len(word_frequency)} unique words from {file.filename}"
        )
        ...
        logger.error(
            f"Error counting word frequency from file {file.filename}: {str(e)}",
            exc_info=True,
        )
        raise HTTPException(status_code=500, detail=f"Error processing file: {str(e)}")
