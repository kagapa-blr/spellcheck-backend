...
from sqlalchemy import exists, delete, func, select
...
class MainDictionaryService:
                ...
                for w, f in aggregated.items():
                    ...
                    if lw in existing_lower_set:
                        db.query(MainDictionary).filter(
                                ...
                                ).update({MainDictionary.frequency: MainDictionary.frequency + f},
    ...
    def clean_dictionary(
        ...
        # cleaned_word -> survivor row id
        seen_words: dict[str, int] = {}

        while True:
            items = (
                    db.query(MainDictionary.id, MainDictionary.word, MainDictionary.frequency)
                    .filter(MainDictionary.id > last_id)
                    .order_by(MainDictionary.id.asc())
                    .limit(batch_size)
                    .all()
            )
            ...
            rows_to_update_word: dict[int, str] = {}
            freq_to_add: dict[int, int] = {}
            ...
            for item in items:
                ...
                try:
                    cleaned_word = clean_kannada_word(original_word)
                except Exception:
                    cleaned_word = None

                # invalid word after cleaning
                if not cleaned_word:
                ...
                survivor_id = seen_words.get(cleaned_word)

                # duplicate after normalization
                if survivor_id is not None:
                    freq_to_add[survivor_id] = freq_to_add.get(survivor_id, 0) + item.frequency
                    ids_to_delete.append(item.id)
                ...
                # first occurrence becomes survivor
                seen_words[cleaned_word] = item.id

                if cleaned_word != original_word:
                    rows_to_update_word[item.id] = cleaned_word
                    updated_count += 1

            try:
                # apply cleaned word updates first for survivor rows
                for row_id, cleaned_word in rows_to_update_word.items():
                    db.query(MainDictionary).filter(
                            MainDictionary.id == row_id
                            ).update(
                            {MainDictionary.word: cleaned_word},
                            synchronize_session=False,
                            )

                # merge duplicate frequencies into survivor rows
                for survivor_id, add_freq in freq_to_add.items():
                    db.query(MainDictionary).filter(
                            MainDictionary.id == survivor_id
                            ).update(
                            {MainDictionary.frequency: MainDictionary.frequency + add_freq},
                            synchronize_session=False,
                            )

                # delete invalid and duplicate rows
                if ids_to_delete:
                    db.execute(
                            delete(MainDictionary).where(MainDictionary.id.in_(ids_to_delete))
                            )
                ...
                db.rollback()
                logger.exception("Dictionary cleanup failed")
                raise
